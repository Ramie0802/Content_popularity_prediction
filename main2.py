...
from dataset_processing import sampling_mobility,  preprocessing_lstm_semantic_branch 

from predict import test_lstm

from model import  LSTMModel
from local_update import LocalUpdate_branch2
...
if __name__=='__main__':
    ...
    ,
            ...
            vehicle_model_dict2[veh].append(copy.deepcopy(local_net2))# cho nay phai copy weight cuar cluster
        ...
        for veh in range(args.clients_num):
            vehicle_seq = veh
            recommend_list = test_lstm(vehicle_model_dict2[veh][-1], sample_lstm_branch, users_group_test[vehicle_seq])
            recommend_list500 = get_top_items(recommend_list)
            recommend_movies_c500.extend(list(recommend_list500))
        print(f"List recommed movies at BS at round {idx + 1}", recommend_movies_c500)
