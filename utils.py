...
def concate3branch(df1, df2, df3):
    ...
    df1_renamed = df1[['movie_id', 'popularity']].rename(columns={'popularity': 'popularity1'})
    df2_renamed = df2[['movie_id', 'popularity']].rename(columns={'popularity': 'popularity2'})
    df3_renamed = df3[['movie_id', 'popularity']].rename(columns={'popularity': 'popularity3'})

    concatenated_df = pd.concat([df1_renamed.set_index('movie_id'), df2_renamed.set_index('movie_id'), df3_renamed.set_index('movie_id')], axis=1)
