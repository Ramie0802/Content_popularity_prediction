...
def sampling_mobility(args, vehicle_num):
    ...
    try:
        ...
        for i in range(vehicle_num):
            users_num_client[i] = int(
                (user_info.index[-1] + 1) * users_num_client[i] / a
            )
        ...
        for i in range(vehicle_num):
            ...
            index_end = ratings[
                ratings["user_id"] == user_seq_client[i] + users_num_client[i]
            ].index[-1]
            users_group_all[i] = set(np.arange(index_begin, index_end + 1))

            NUM_train = int(0.998 * len(users_group_all[i]))

            users_group_train[i] = set(
                np.random.choice(list(users_group_all[i]), NUM_train, replace=False)
            )
        ...
        for i in range(args.epochs):
            for j in range(vehicle_num):
                if j == 0:
                    ...
                    request_content[i] = np.random.choice(
                        list(users_group_test[j]),
                        int(all_test_num / args.epochs * users_num_client[j] / a),
                        replace=True,
                    )
                    vehicle_request_num[i].append(
                        int(all_test_num / args.epochs * users_num_client[j] / a)
                    )
                else:
                    request_content[i] = np.append(
                        request_content[i],
                        np.random.choice(
                            list(users_group_test[j]),
                            int(all_test_num / args.epochs * users_num_client[j] / a),
...
def preprocessing_lstm_semantic_branch(sample):
    ...
    dataset["semantic_vector"] = [
        id_to_vector.get(str(id_val)) for id_val in dataset["movie_id"]
    ]
    dataset = pd.concat(
        [
            dataset.drop("semantic_vector", axis=1),
            dataset["semantic_vector"].apply(pd.Series),
        ],
        axis=1,
    )
